...
export function generateCardHtml(config: CardConfig): string {
  const { colors, font, layout, name, role, location, phone, whatsapp, email, website, socials, portrait, qr } = config;
  ...
  const portraitStyle = layout === 'bold' ? 'aspect-ratio:16/10;' : '';
